...
import { BAD_INPUT, CHANGED, ERROR_MSG } from "@/assets/messages";
...
export const removeUser = async (userId: string) => {
	try {
		...
		return [
			"Zkuste znovu",
			"V případě častého vyskytnutí problému kontaktujte vývojáře",
		];
	}
};
...
export const updateUser = async (
	...
	const roleChanged =
		values.role === user.role_name || !values.role ? false : true;
	...
	const somethingChanged =
		emailChanged || nameChanged || emailVerChange || roleChanged;
