    ...
    startTransition(() => {
      updateUser(user, values)
        .then((data) => {
          ...
          setSuccess(data?.success + ". Pro projevení změn je nutné reloadnout stránku.");
          // setError(data?.error)
        })
    })
  }
    ...
    <>
      <Form {...form}>
        <form
          ...
          className="space-y-4"
        >
          <h1 className="font-primary font-semibold text-xl">Editace uživatele {user.name}</h1>
          <FormField
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {roles?.map((role, index) => (
                      <SelectItem key={index} value={role.name}>{role.name}</SelectItem>
                    )
                    )}
                  </SelectContent>
                </Select>
